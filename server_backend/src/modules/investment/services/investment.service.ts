    ...
    return this.investmentRequestService
      ...
      .pipe(
        tap((investmentRequest) => {
          ...
          if (createInvestmentDto.amount % 10 !== 0) {
            throw new HttpException(
              'The amount must be a multiple of 10 USD.',
              HttpStatus.BAD_REQUEST,
            );
          }
          if (
        ...
        mergeMap((data) => {
          ...
          errorData = data.ErrorInformation;
          ...
          if (commandStatus == CommandStatus.ERROR) {
            ...
            return this.usersServie.findOne(payload.userId).pipe(
              mergeMap((user) => {
                ...
                throw new HttpException(
                  `Your Payment was declined. You will be fined ${data.globalSettings.invalidTransactionPenalty} USD.`,
                  HttpStatus.BAD_REQUEST,
                );
              }),
            );
