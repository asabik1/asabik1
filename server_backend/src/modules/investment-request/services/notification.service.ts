...
import { mergeMap, zip, reduce, Observable, of, catchError } from 'rxjs';
  ...
  sendPlaidVerificationNotifications(): void {
    ...
    this.usersService
      ...
      .pipe(
        ...
        reduce(() => {}),
        catchError((err) => {
          this.logger.error('sendPlaidVerificationNotifications');
  ...
  sendPaymentNotifications(): void {
    ...
    this.usersService
      ...
      .pipe(
        ...
        reduce(() => {}),
        catchError((err) => {
          this.logger.error('sendPaymentNotifications');
          throw new HttpException(err, HttpStatus.BAD_REQUEST);
        }),
      )
      .subscribe();
