...
import { PaymentType } from '../../plaid/enum/payment-type.enum';
...
export class LoanService {
  ...
  processAcceptLoan(
    ...
    amountToBeDeposited -= globalSettings.applicationFee;

    return this.sendFundsToBusinessOwner(
      investmentRequest.businessOwner,
      amountToBeDeposited,
    ).pipe(
      mergeMap((payment) =>
        this.createLoan({
  ...
  sendFundsToBusinessOwner(
    ...
    return forkJoin({
      ...
      adminUserIdentity: this.usersService
        .getAdmin()
        .pipe(
          mergeMap((adminUser) => this.plaidService.getIdentity(adminUser.id)),
        ),
    }).pipe(
      mergeMap((data) =>
        this.achqService.submitPayment(
          {
            ...
            SECCode: 'WEB',
            paymentAmount: amountToBeDeposited,
