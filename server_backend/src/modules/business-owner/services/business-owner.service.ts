...
export class BusinessOwnerService {
  ...
  decideInvestmentRequestForManualProcessing(
    ...
    return forkJoin({
      investmentRequest: this.investmentRequestService.findOneOrFail(
        decideInvestmentRequest.investmentRequestId,
      ),
      globalSettings: this.globalSettingsService.getGlobalSettings(),
    }).pipe(
      tap((data) => {
        if (
          data.investmentRequest.status !==
      ...
      mergeMap((data) =>
        this.investmentRequestService.approveInvestmentRequestFromManualProcessing(
          ...
          data.globalSettings.raisingTimeLimit,
        ),
      ),
    );
  }
