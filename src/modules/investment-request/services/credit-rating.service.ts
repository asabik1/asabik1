...
export class CreditRatingService {
  ...
  calculateCreditRating(
    ...
    const d1: number =
      (Math.log(I / O) + (this.globalSettingsService.R + VI ** 2 / 2) * Y) /
      (VI * Y ** 0.5);
    const d2: number = d1 - VI * Y ** 0.5;
    const d0: number =
      d2 * this.globalSettingsService.M + this.globalSettingsService.A;
    const PoD: number = this.calculateNormSDist(-d0);

    return forkJoin({
      maxLoanCalc: this.calculateMaxLoan(Y, VI, E, L),
            ...
            (12 * E * Y) /
              Math.exp(
                (-1 * this.normSInv(crb) + VI * Y ** 0.5) * (VI * Y ** 0.5) -
                  (this.globalSettingsService.R + VI ** 2 / 2) * Y,
              ) -
              12 * L * Y,
          ),
        ),
  ...
  updateInvestmentRequestsRatings(): void {
    ...
    this.investmentRequestService
      .findAllAcceptedInvestmentRequests()
      .pipe(
  ...
  updateInvestmentRequestsFromSurvey() {
    ...
    try {
      this.businessOwnerRepository
        .findAllBusinessOwners()
        .pipe(
          concatMap((businessOwners) =>
            from(businessOwners).pipe(
