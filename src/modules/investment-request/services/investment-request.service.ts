...
export class InvestmentRequestService {
  ...
  acceptLoan(
    ...
    return this.findOneOrFail(investmentRequestId).pipe(
      ...
      mergeMap((investmentRequest) =>
        this.loanService.processAcceptLoan(investmentRequest, payload),
      ),
      mergeMap(() => {
  ...
  generateCreditRatingAndSaveNewInvestmentRequest(
    ...
    if (
      investmentRequestDto.requiredCapital < this.globalSettingsService.MIN_LOAN
    ) {
      throw new HttpException(
      ...
      investmentRequestDto.requiredCapital > this.globalSettingsService.MAX_LOAN
    ) {
      throw new HttpException(
    ...
    const expiresAt = new Date();
    expiresAt.setDate(
      now.getDate() + this.globalSettingsService.RAISING_TIME_LIMIT,
    );

    return this.creditRatingService
      .calculateCreditRating(financialReport, investmentRequestDto.returnTerm)
      .pipe(
        tap((creditRating) => {
          if (
            creditRating.maxLoanCalc < this.globalSettingsService.MIN_LOAN ||
        ...
        mergeMap((creditRating) => {
          const creditRatingData = {
            ...
            r: this.globalSettingsService.R,
            m: this.globalSettingsService.M,
            a: this.globalSettingsService.A,
          ...
          if (
            ...
            this.globalSettingsService.MAX_RETURN_TERM_WO_MANUAL_PROCESSING
          ) {
  ...
  createInvestmentRequest(
    ...
    return forkJoin({
      ...
      tap((data) => {
        ...
        } else if (data.businessOwner.monthlyReports === null) {
          throw new HttpException(
            MESSAGE_TO_BUSINESS_OWNER.FINANCIAL_REPORT_CONDITIONS_NOT_MET,
            HttpStatus.BAD_REQUEST,
          );
  ...
  approveInvestmentRequestFromManualProcessing(
    ...
    expiresAt.setDate(
      approvedAt.getDate() + this.globalSettingsService.RAISING_TIME_LIMIT,
    );
